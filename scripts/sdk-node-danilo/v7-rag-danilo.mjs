...
// [RAG] ────────────────────────────────────────────────────────────────
const KB_ID = process.env.KB_ID ?? "SEU_KB_ID"; // KB com S3 Vectors
const TOP_K = 5; // quantos trechos trazer por busca
...
console.log("🤖 Chatbot de Venda de Ingressos - LAB AWS Danilo");
console.log("================================================");
console.log("");
...
if (LOG) {
  writeFileSync(
    LOG,
    `${"═".repeat(78)}\n` +
    `🤖 SESSÃO RAG V7 INICIADA  ·  ${new Date().toLocaleString("pt-BR")}\n` +
    `📚 Knowledge Base: ${KB_ID} | Top K: ${TOP_K} | Score mínimo: ${SCORE_MINIMO}\n` +
    `${"═".repeat(78)}\n\n`
  );
